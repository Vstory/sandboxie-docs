...
_DropChildProcessToken_ 是一个沙箱设置项，自 v1.15.6 / 5.70.6 起在 [沙盘 Ini](SandboxieIni.md) 中可用。此设置强制指定应用程序的子进程在运行时不使用受修改的安全令牌，从而绕过沙盘通常的受限令牌机制。该功能主要作为调试工具，用于排查“绿框”（应用程序隔离）兼容性问题，即应用程序因令牌限制而无法正常启动或运行。
